...
db2.prototype.applyPagination = function(model, stmt, filter) {
  ...
  var paginatedSQL = 'SELECT * FROM ( ' + stmt.sql + ' ) WHERE rownum > ' + offset;
...
db2.prototype.buildColumnNames = function(model, filter) {
  ...
  if (filter.limit || filter.offset || filter.skip) {
    ...
    columnNames += ',ROW_NUMBER() OVER' + ' (' + orderBy + ') rownum';
  }
  return columnNames;
};

db2.prototype.buildSelect = function(model, filter, options) {
  if (!filter.order) {
